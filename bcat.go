...
func stdinExposeHandler(w http.ResponseWriter, r *http.Request) {
...
func embeddedJSONViewHandler(w http.ResponseWriter, r *http.Request) {
...
func main() {
	...
	flag.BoolVar(&json, "json", false, "set Content-Type to application/json")
	flag.BoolVar(&jsonview, "jsonview", false, "Open with embedded JSONView")
	...
	if !jsonview {
		http.HandleFunc("/", stdinExposeHandler)
	} else {
		http.HandleFunc("/", embeddedJSONViewHandler)
	}
	...
	go func() {
		...
		case <-time.After(time.Second):
			if failed != true {
				browser.OpenURL("http://localhost:" + strconv.Itoa(port))
			}
		}
	}()
